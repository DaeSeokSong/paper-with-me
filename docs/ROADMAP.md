...
- [x] 리더보드 Extra Training Data 컬럼 + 데이터셋 표기 변형(variants)
      매칭 (재빌드 스냅샷부터 데이터 반영)
- [x] 리더보드 2025-07 이후 갱신 자동화 — 일일 수집 논문의 초록에서
      기존 벤치마크·지표·범위 안의 수치만 보수적으로 추출해 source='auto'
      행으로 추가 (`pwc collect --source results`). 자동 행은 주 지표 값
      순서로 보드에 병합되고 "자동 추출" 배지로 구분, 커뮤니티 기여
